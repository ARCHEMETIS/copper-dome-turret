...
- [x] Calibrate รอบฮาร์ดแวร์จริง: จูนทิศ/ลิมิต servo จากเฟือง rack จริง (`TILT_CENTER=175 / TILT_MIN=90 / TILT_MAX=180`), ตั้งจุด zero ของสโคป
...
1. **ตั้งจุด zero ของสโคปจากการยิงจริง** — ก่อนตั้ง ระบบเล็งแม่นแต่ยิงพลาดเป็นระบบทุกนัด เพราะแกนกล้องกับแกนลำกล้องไม่ทับกัน เป็นตัวแปรเดียวที่กระทบคะแนนมากกว่าเรื่องโมเดลทั้งหมดรวมกัน
2. **แก้บั๊ก "คลิกคาปิบาร่าแล้วป้อมหันไปหาช้าง"** (29 ก.ค.) — ช้างตัวเดียวถูกอ่านซ้อนสองชื่อ แล้วเส้นทางเล็งเลือกจาก conf สูงสุดแทนตัวที่คนชี้ แก้ด้วย `suppress_conflicts()` + ส่งพิกัดที่คลิกลงไปเป็น `anchor` ดูรายละเอียดใน `src/detector.py`
...
4. **ยิงวิถีแบนแรงคงที่** แทนการคำนวณวิถีโค้งตามระยะ — ลดตัวแปรที่ต้องแม่นพร้อมกันจากหลายตัวเหลือตัวเดียว ระบบ ranging จึงเหลือบทบาทแค่ HUD + ประตูกันกรอบขนาดเพี้ยน
