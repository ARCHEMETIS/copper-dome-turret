...
>
> ARIS Project III — Mini-project 1 | แข่ง 30 ก.ค. 2026 — **ผลจริง: ยิงโดน 5/5 เป้า** ([สรุปปิดโปรเจค](#-ผลการแข่งขัน--ปิดโปรเจค))
...
- [x] ประกอบฮาร์ดแวร์ + พิมพ์ชิ้นส่วน 3D (ใช้ TT motor เดิม ถอด gearbox ออก ใช้แกนมอเตอร์เปลือย แรงพอยิงลูกบอลไม้ได้จริงเกิน 1.5 m)
- [x] Calibrate รอบฮาร์ดแวร์จริง: จูนทิศ/ลิมิต servo จากเฟือง rack จริง (`TILT_CENTER=175 / TILT_MIN=90 / TILT_MAX=180`)
- [ ] เขียนค่าจุด zero ของสโคปลง `config.py` ให้ถาวร — ดู "ข้อจำกัดที่ยังค้าง" ด้านล่าง
- [x] ทดสอบรวมระบบ + เก็บสถิติความแม่น
- [x] นำเสนอ 30 ก.ค. 2026

## 🏁 ผลการแข่งขัน — ปิดโปรเจค

**ยิงโดนครบ 5/5 เป้า** (เกณฑ์ผ่านคือ 5 จาก 10 นัด) — โปรเจคปิดที่ tag `v1.0-final`

สิ่งที่ตัดสินผลจริงหน้างาน เรียงตามน้ำหนัก:

1. **แก้บั๊ก "คลิกคาปิบาร่าแล้วป้อมหันไปหาช้าง"** (29 ก.ค.) — ช้างตัวเดียวถูกอ่านซ้อนสองชื่อ แล้วเส้นทางเล็งเลือกจาก conf สูงสุดแทนตัวที่คนชี้ แก้ด้วย `suppress_conflicts()` + ส่งพิกัดที่คลิกลงไปเป็น `anchor` ดูรายละเอียดใน `src/detector.py`
2. **ยิงวิถีแบนแรงคงที่** แทนการคำนวณวิถีโค้งตามระยะ — ลดตัวแปรที่ต้องแม่นพร้อมกันจากหลายตัวเหลือตัวเดียว ระบบ ranging จึงเหลือบทบาทแค่ HUD + ประตูกันกรอบขนาดเพี้ยน
3. **hard negative จากคลิปห้องจริง** (`train-5`) — ตัด false positive จาก 8.0% เหลือ 1.1% โดย recall ไม่ตก (95.7% → 96.0%) บทเรียนคือ *dataset ใหญ่ไม่ช่วยถ้า negative แคบ*
4. **จูนทิศและลิมิต servo จากเฟือง rack จริง** — ก่อนจูน ป้อมหนีเป้าทั้งสองแกน และ `TILT_CENTER` เดิมอยู่ที่ขอบ servo พอดีจนเสี่ยงไฟตก

**ข้อจำกัดที่ยังค้างไว้ตามจริง** (ไม่ได้แก้ ไม่ใช่ว่าไม่มี):
- 🔴 **จุด zero ของสโคปไม่เคยถูกบันทึกลง `config.py`** — `SCOPE_ZERO_OFFSET_PX` ยังเป็น `[0, 0]` มาตั้งแต่คอมมิตที่สร้างมัน ค่าที่ปรับสดหน้างานผ่านปุ่มบน UI อยู่ในหน่วยความจำอย่างเดียว (โปรแกรมขึ้นเตือน `WRITE IT INTO config.py !` ทุกครั้งที่ปรับ) หายไปตอนปิดโปรแกรม ⇒ **clone ใหม่มารันจะเริ่มที่ศูนย์เสมอ ต้อง zero ใหม่ทุกครั้งก่อนยิง**
- ตัวเลข false positive 1.1% วัดจาก background ที่เป็นห้องผู้ใช้ 100% — **ไม่ใช่ตัวเลขของห้องแข่ง** เฟรมห้องแข่งอยู่ใน train หมด valid/test จึงวัด FP ห้องแข่งไม่ได้ตามนิยาม
- `Distance/ranging_log.csv` ยังเป็นข้อมูลของโมเดลรุ่นก่อน ขา ranging ใน smoke test จึงข้ามตัวเอง — ไม่กระทบการยิงเพราะวิถีแบนไม่ใช้ ranging
- L298N ช่อง OUT1/OUT2 พังครึ่งเดียว (ขับได้ทิศเดียว) แก้เฉพาะหน้าด้วยการสลับสายมอเตอร์จริง ไม่ได้เปลี่ยนบอร์ด
