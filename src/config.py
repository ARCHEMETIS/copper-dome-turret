...
# กลไกยิง = หน้าไม้ (ตัดสินใจ 18 ก.ค. — ดูเหตุผลใน docs/แผนโปรเจค-CopperDome.md §2.5)
# ไม่ใช่ flywheel เดิมแล้ว: servo ตัวที่ 2 ย้ายจาก feeder → tilt, มอเตอร์ DC 2 ตัว
# ย้ายจาก flywheel → มอเตอร์ดึงสายหน้าไม้ (ใช้ของเดิมทั้งหมด ไม่ซื้อเพิ่ม)
PIN_PAN_SERVO = 9           # MG945 ตัวที่ 1 หมุนป้อมซ้าย-ขวา
PIN_TILT_SERVO = 10         # MG945 ตัวที่ 2 คุมมุมเงย ผ่านเฟืองสะพาน (rack) ใต้ตัวป้อม
                            # (เดิมเป็น feeder servo — หน้าไม้ไม่ต้องมี feeder ดันลูกเข้าล้อแบบ flywheel)
PIN_DRAW_ENA = 5            # PWM -> ENA ของ L298N (มอเตอร์ดึงสายตัวที่ 1) เดิมคือ flywheel ENA
PIN_DRAW_ENB = 6            # PWM -> ENB ของ L298N (มอเตอร์ดึงสายตัวที่ 2) เดิมคือ flywheel ENB
PIN_IN1 = 2                 # ทิศทางมอเตอร์ดึง A (ตั้งครั้งเดียวตอนเปิดเครื่อง)
PIN_IN2 = 4
PIN_IN3 = 7
PIN_IN4 = 8
# หมายเหตุ: มอเตอร์ดึงสาย 2 ตัวต้องหมุน "ทิศเดียวกัน" (ดึงสาย/พันเฟืองครึ่งวงเข้าหาตัว)
# ต่างจาก flywheel เดิมที่ต้องสวนทางกัน — ถ้าดึงไม่เข้าหรือหมุนผิดทาง สลับคู่ IN ที่ตัวนั้น (ดู hardware.py)
PIN_CAM_SWITCH = 12         # digital input (internal pull-up ของ Arduino — ไม่ต้องมีตัวต้านทานเพิ่ม)
...
# กระแสหลายเท่าของตอนหมุนปกติ ทำแบตวูบจน L298N ดับทั้งบอร์ด (เจอของจริง 15 ก.ค., เดิมเจอกับ flywheel)
DRAW_RAMP_STEP = 0.05        # ขยับทีละกี่ duty ต่อขั้น
