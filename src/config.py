...
# ตัวแก้ระยะตามท่า (ranging.py ใช้): {toy: [(aspect_min, aspect_max, factor), ...]}
# aspect = w/h ของกรอบ YOLO — บอก "ท่า" ได้: เช่น dino ยืนตรง w/h~0.65,
# หันข้าง ~1.0 (เงาใหญ่ อ่านใกล้เกิน -20% → คูณ 1.25 ชดเชย)
# fit จาก Distance/ranging_log.csv (โมเดลรอบ 3, 42 จุด) — median err ต่อแถบ
# หลังแก้: ทุกจุดเข้า ±10% (เดิมพลาดถึง ±21%) แต่เป็นการวัดบนข้อมูลชุดเดียว
# กับที่ fit — เช็คของจริงผ่าน tools/test_on_webcam.py อีกชั้นก่อนเชื่อสนิท
# ⚠ ผูกกับโมเดล+ตุ๊กตาชุดนี้ ถ้าเทรนใหม่/เปลี่ยนตุ๊กตา ต้อง fit ใหม่พร้อม real_size_mm
ASPECT_CORRECTION = {
    "capybara": [(0.0, 0.95, 1.036), (0.95, 9.9, 0.875)],
    "dino":     [(0.0, 0.72, 1.012), (0.72, 0.90, 0.902),
                 (0.90, 1.05, 1.254), (1.05, 9.9, 0.959)],
    "elephant": [(0.0, 1.05, 1.045), (1.05, 9.9, 0.906)],
}
