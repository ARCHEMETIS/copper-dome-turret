...
# วันแข่งมุมเงย + โต๊ะบังท่อนล่างของเป้า → โมเดลอาจมองไม่เห็น/กรอบเพี้ยน
# คนเลือกเป้าเองอยู่แล้ว จึงเล็งด้วย "คลิกที่ไหนก็หันไปที่นั่น" ไม่พึ่ง detection:
# หมุนป้อมเอาคอลัมน์พิกเซลที่คลิกมากลางจอ (มุม = atan(offset_px / FOCAL_PX))
CLICK_AIM_GAIN = 1.0        # คูณมุมที่หมุนต่อคลิก (<1 = ค่อยๆ เข้า กันเหวี่ยงเกินถ้า FOCAL_PX ไม่เป๊ะ)
# ระยะยิงแบบเลือกเอง — กด 1/2/3 (เพราะโต๊ะบังทำให้ ranging จากขนาดกรอบใช้ไม่ได้)
# สนามจริง 1.5-2.4m → ค่านี้ป้อนเข้า ranging.duty_for_distance เหมือน ranging ปกติ
RANGE_PRESETS_MM = {"near": 1500, "mid": 2000, "far": 2400}
RANGE_DEFAULT = "mid"
